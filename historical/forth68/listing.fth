\ ====================================================================
\ Moore's 1968 FORTH listing -- the FORTH-source overlay loaded by
\ the kernel on top of the assembly primitives. Translated from
\ reference/1968-FORTH/FORTH68lst.txt under Moore's May-2020 public-
\ posting permission (see ../NOTICE).
\
\ Translation policy: preserve Moore's source verbatim modulo:
\   * comment delimiter '\' added at translation time (the original
\     listing had no inline comments; section dividers added here
\     for navigation only).
\   * Moore wrote '.WORD body ,' (dot-comma) for definitions; we
\     preserve that syntax. A future modernisation pass could
\     translate to ':' / ';' for readability.
\   * Algorithm, identifier names, and definition order: unchanged.
\
\ Status: **documentation-only.** This file is not yet executable;
\ our kernel's ACCEPT mechanism reads characters via LIBF DISK1,
\ which is stubbed (see ../RUNTIME-STATUS.md). When a future saga
\ wires real disk I/O or a memory-buffer input source, this
\ listing becomes loadable as the kernel's bootstrap dictionary
\ extension.
\
\ The listing's logical sections (added by us for navigation):
\   1. Instruction-template builders (LD, ST, ADD, ...) lines 5-22.
\   2. Code-generation helpers (GDEP, REL, BASE, ...) lines 24-36.
\   3. CONSTANT / REVERT / TOP / DUP / VALUE / DROP / SWAP / ...
\   4. Arithmetic primitives (+, -, *, /, AND, MINUS).
\   5. PUSH / PULL / EXECUTE / control-flow words.
\   6. Conditional family (CONDITION, NONZERO, FALSE, EVEN, IF, THEN).
\   7. ENQUEUE / REACTIVATE / interrupt + queue infrastructure.
\   8. CSKB / CSCP character tables.
\   9. Console I/O (READY, CHA, CHARACTER, ACCEPT, TYP, RIBBON, TYPE).
\  10. Disk I/O (CYLINDER, SECTOR, WRITE).
\  11. File system (REP, FILL, LIZ, ', (, $, FILE, LINE, ...).
\  12. Top-level entry: LOC DONE 0A= HOME 'HI THERE' REPLY CONSOLE.
\ ====================================================================

\ -- section 1: instruction-template builders --------------------------
...
\ -- section 3: dictionary management + stack primitives ---------------
...
\ -- section 5: PUSH / PULL / EXECUTE / control-flow -------------------
...
\ -- section 6: CONDITION / IF / THEN / ELSE family --------------------
...
\ -- section 7: ENQUEUE / REACTIVATE / interrupt + queue ---------------
...
\ -- section 8: CSKB and CSCP character tables -------------------------
...
\ -- section 11: file system + REPL helpers ---------------------------
